...
class Preprocces:
    ...
    def fill_categorical_na(self, columns, df, knn=False):
        ...
        if not knn:
            ...
            data_cat_imputed = imputer.fit_transform(
                df_result.drop(['account_uuid', 'product', 'state',
    ...
    def clean_dataframe(self):
        ...
        df[['annual_revenue', 'total_payroll', 'premium']] = df[
            ['annual_revenue', 'total_payroll', 'premium']].applymap(lambda x: np.log(x) if x > 0 else x)
